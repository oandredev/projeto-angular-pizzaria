...
import { History as HistoryT, CartItem } from '../../core/types/types'; // Usando as importações tipadas do usuário
...
export class History implements OnInit { // Implementação do OnInit para carregar dados
  ...
  ngOnInit(): void {
    // Chamada para carregar o histórico quando o componente é inicializado
  ...
  exibirHistorico(): void {
    ...
    this.historyService.getHistoryByUser().subscribe({
      ...
      error: (error: any) => {
        ...
        this.errorMessage = 'Não foi possível carregar o histórico de compras. Verifique a conexão com a API.';
        this.isLoading = false;
